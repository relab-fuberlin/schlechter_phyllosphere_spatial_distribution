...
w2_taxa <- auc_fold_change %>% 
    group_by(type, dpi) %>% 
    wilcox_test(fractional_change ~ taxa_pair, p.adjust.method = "holm", detailed = TRUE) %>% 
    filter(p.adj < 0.05)
...
```{r figure_supplemental_6, warning=FALSE, fig.id=TRUE, fig.cap="Frequency plots g", dev = "png", fig.dim=c(12,8)}
