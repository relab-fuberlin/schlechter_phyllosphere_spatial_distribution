...
By contrast, *Methylobacterium* species responded differently to community complexity. In general, MeL85, MeL92, and Mr0-1 consistently decreased in population sizes, especially in S3 and less pronounced in S2. Within *Methylobacterium*, MeL92 and Mr0-1 benefited from the presence of one other species (S2). This effect was only observed at 14 dpi, and it was lost when a third competitor was present (S3), regardless of the competitor's identity. MeL85 was the most susceptible species to decreasing in population size over time and across different community complexities and compositions (Fig. Sup).

These results indicated that bacterial taxa differentially respond to community complexity in the phyllosphere and that *Methylobacterium* are more affected compared to *Sphingomonas*. The most competitive species was the sphingomonad SmFR1, while the most susceptible was the methylobacterium MeL85.
...
```{r plot sup1 setup, include=FALSE}
...
plt.sup1 = cbind(data_fold_S2, data_fold_S3[,c(-1:-2)]) %>% 
    ...
    scale_size_continuous(range = c(12,3), breaks = c(0.05, 0.5, 1), 
                          label = c("< 0.05", "0.5", "1.0"), name = expression(paste(italic("P"), "-adjusted")))+
    coord_fixed()+
    scale_y_discrete(name="Focal strain", labels = sp.lab)+
