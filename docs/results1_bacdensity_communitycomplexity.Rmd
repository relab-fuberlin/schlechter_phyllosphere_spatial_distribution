...
result_list <- lapply(split_data, process_triad, "cfu")
...
plt_cfu_taxa_pair <- cfu_pair_full %>% 
    filter(synID != "C") %>% 
    ggplot(., aes(taxa_pair, log_cfu, fill = fct_rev(synID)))+
    ...
    geom_text(data = contrast_list, aes(label = .group, y = 9.5, group = fct_rev(synID)), 
...
plt_cfu <- data_cfu %>% 
    ...
    add_pvalue(data = dunn_cfu_taxa, label = "p.adj.signif",
               xmin = "xmin", xmax = "xmax", tip.length = 0.01, size = 2,
...
fc_cfu_taxa <- fc_cfu %>% 
    ...
    add_pvalue(dunn_taxa_dpi, label = "p.adj.signif", 
               ...
               size = 2, lineend = "round", bracket.size = 0.2, coord.flip = TRUE)+
    coord_flip()+
...
# Save plot 
ggsave(here("results", "figure_x5.png"), width = 6, height = 4)
ggsave(here("results", "figure_x5.pdf"), width = 6, height = 4)
