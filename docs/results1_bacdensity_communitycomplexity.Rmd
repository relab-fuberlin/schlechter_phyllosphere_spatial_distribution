...
Changes in population density could be related to temporal changes. Thus, we evaluated how population density changed over the two times of sampling, 7 dpi and 14 dpi. Here we observed a small yet significant effect of time of sampling (Wilcoxon, *Z* = `r w_dpi$statistic`, *p* = `r w_dpi$p.value`, *r* = `r weff_dpi$effsize` [`r weff_dpi$conf.low`-`r weff_dpi$conf.high`]). However, since populations only increased `r fc_cfu_dpi$FC[fc_cfu_dpi$dpi=="14dpi"]`-fold between 7 and 14 dpi, and considering that overall changes were consistent, we continued our analysis by grouping the data collected from each sampling point.
...
cbind(data_fold_S2, data_fold_S3[,c(-1:-2)]) %>% 
