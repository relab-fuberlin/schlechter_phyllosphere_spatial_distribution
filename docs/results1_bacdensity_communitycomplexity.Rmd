...
patch <- ( plt1 / plt2 ) + plot_layout(guides="collect", heights = c(2,5))
```
...
plt.cfu.stat = dunn_synID %>% mutate(p.adj.signif = case_when(p.adj.signif == "****" ~ "*", TRUE ~ p.adj.signif),
...
```{r}
plt.cfu + patch + plot_annotation(tag_levels = "A")
```
