...
wrap_elements(plot = plt3.a) + plt3.b + plt3.c  + 
    ...
    plot_layout(design = areas, guides = "collect") & 
...
plt.sup1 = cbind(data_fold_S2, data_fold_S3[,c(-1:-2)]) %>% 
    ...
    facet_grid(dpi~label, labeller = labeller(dpi = dpi.lab2), scales = "free_x")+
    geom_tile(colour = "black", fill = "white", linewidth = 0.1)+
    ...
    scale_size_continuous(range = c(10,3), breaks = c(0.05, 0.5, 1), 
                          label = c("< 0.05", "0.5", "1.0"), 
                          name = expression(paste(italic("P"), "-adjusted")))+
    scale_y_discrete(name="Focal strain", labels = sp.lab)+
...
plt.sup1
ggsave(here("results", "plot_cfu_sup.png"), width = 10, height = 5)
```
