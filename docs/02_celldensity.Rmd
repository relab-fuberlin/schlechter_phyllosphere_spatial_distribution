...
```{r}
# Split the data
split_data <- split(data_cell,
          list(data_cell$synID, data_cell$syncom, data_cell$exp, data_cell$rep, data_cell$dpi), 
          drop = TRUE, sep = "_")
...
contrast_list <- lapply(emm_list, cld, alpha = 0.05, Letters = letters, adjust = "holm") %>% 
    lapply(., data.frame) %>% 
    do.call(rbind, .) %>% 
...
plt_cell_taxa_pair <- cell_pair_full %>% 
    filter(synID != "C") %>% 
    ggplot(., aes(taxa_pair, log_cell, fill = fct_rev(synID)))+
    facet_grid(cols = vars(dpi), labeller = labeller(dpi = dpi.lab2))+
    geom_jitter(aes(group = fct_rev(synID)), 
    ...
    scale_fill_manual(name = "SynCom", values = syn.pal[2:3], breaks = syn.lab[2:3])+
    theme_rs()+
    coord_flip()+
    labs(y = plt_bac_cell_density_lab,
         x = "Taxa Pair")+
...
contrast_pair <- lapply(emm_pair, contrast, alpha = 0.05, "pairwise", adjust = "holm") %>% 
    lapply(., data.frame) %>% 
    do.call(rbind, .) %>% 
    rownames_to_column(var = "label") %>% 
...
plt_cell_strain_pair <- cell_pair_full %>% 
    ...
    geom_jitter(aes(fill = fct_rev(synID)), 
                position = position_jitterdodge(jitter.width = 0.45, dodge.width = 0.9), 
                alpha = 0.2, size = 0.5)+
    geom_boxplot(aes(fill = fct_rev(synID)), outlier.alpha = 0, alpha = 0.9, size = 0.2, width = 0.9)+
...
dunn_strain_pair <- pair_S3 %>% 
    group_by(dpi) %>% 
    dunn_test(fc_pair ~ pair, p.adjust.method = "holm") %>% 
...
dunn_allpair <- pair_S3 %>% 
    group_by(dpi) %>% 
    dunn_test(fc_pair ~ taxa_pair, p.adjust.method = "holm") %>% 
    mutate(p.adj.signif = ifelse(p.adj < 0.05, "*", "ns")) %>% 
...
plt_taxapair <- pair_S3 %>% 
    ...
    theme_rs()+
    scale_y_continuous(name = bquote(Log[2]~"FC Cell density taxa pair"), limits = c(-3, 2))+
...
plt_cell <- data_cell %>% 
    ...
    add_pvalue(data = dunn_cell_taxa, label = "p.adj.signif",
               xmin = "xmax", xmax = "xmin", tip.length = 0.01, size = 2,
               hide.ns = TRUE, lineend = "round", bracket.size = 0.2, coord.flip = TRUE)+
...
dunn_taxa_dpi <- fc_cell %>% 
    mutate(synID = fct_rev(synID)) %>% 
    group_by(taxa, dpi) %>% 
    dunn_test(fold ~ synID, p.adjust.method = "holm") %>% 
    mutate(p.adj.signif = ifelse(p.adj < 0.05, "*", "ns")) %>% 
    add_xy_position(x = "taxa", dodge = 0.45) %>% 
    filter(p.adj.signif != "ns")
...
fc_cell_taxa <- fc_cell %>% 
    ...
    scale_y_continuous(name = bquote(Log[2]~"FC Bacterial cell density"), 
                       limits = c(-6, 3))+
    scale_x_discrete(name = "", labels = taxa.lab)+
    scale_fill_manual(name = "SynCom", values = syn.pal[2:3], labels = syn.lab[2:3], 
                      limits = c("S2", "S3"))+
    guides(fill = "none")+
    theme_rs()+
    coord_flip()+
...
fc_cell_strain <- fc_cell %>% 
    ...
    add_pvalue(dunn_cell_strain, label = "p.adj.signif", 
               xmin = "xmax", xmax = "xmin", tip.length = 0.01, size = 2,
               hide.ns = TRUE, lineend = "round", bracket.size = 0.2, coord.flip = TRUE)+
    coord_flip()+
...
# Save plot 
ggsave(here("results", "figure_x7.png"), width = 6, height = 4)
ggsave(here("results", "figure_x7.pdf"), width = 6, height = 4)
