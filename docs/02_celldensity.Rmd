...
dunntest_taxa <- dunnet_function(data_cell, "logCell", group, explanatory) %>% 
    mutate(symbol = ifelse(p_label == "< 0.05", "*", "")) %>% 
    rename(synID = group2)

fold_taxa <- fold_change(df = data_cell, 
                         value = "cell_density",
                         ref_variable = explanatory,
                         ref_group = "C",
                         group_by_variable = group)

# Fold change of population density by strain and synID
dunntest_strain <- dunnet_function(data_cell, "logCell", strain, explanatory) %>% 
    mutate(symbol = ifelse(p_label == "< 0.05", "*", "")) %>% 
    rename(synID = group2)

fold_strain <- fold_change(df = data_cell, 
                         value = "cell_density",
                         ref_variable = explanatory,
                         ref_group = "C",
                         group_by_variable = strain)
