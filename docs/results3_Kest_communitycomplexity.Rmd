...
plt.taxa <- w1_taxa_plot_data %>% 
    ...
    scale_fill_gradientn(name = "Change in\naggregation (%)",
        colours = wes_palette("Zissou1")[c(1,2,3)], 
        values=c(0, 0.6, 1), 
        limits = c(-70, 0), 
        breaks = seq(-70, 0, 20))+
    labs(x="", y="")+
...
wrap_elements(plot = plt.frq) + plt.taxa + plt.strain + guide_area() +
...
# Save plot
ggsave(here("results", "plot_K.png"), width = 12, height = 5)
...
```{r figure_s3, warning=FALSE, fig.id=TRUE, fig.cap="Frequency plots K", dev = "png", fig.dim=c(12,8)}
...
# Save plot
ggsave(here("results", "plot_s3.png"), width = 12, height = 8)

```

```{r figure_s4, fig.id=TRUE, fig.cap="Spatial patterns", dev = "png", fig.dim=c(6,6)}
...
# Save plot
plt.pattern
ggsave(here("results", "plot_s4.png"), width = 6, height = 5)
