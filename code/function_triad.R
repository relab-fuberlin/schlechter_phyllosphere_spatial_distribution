...
process_triad <- function(subset_df, cell) {
    ...
    if (length(strain) == 3) {
        ...
        for (combo in comb3) {
            ...
            pair_combinations$tritagonist <- sapply(1:nrow(pair_combinations), function(i) {
...
process_triad_split <- function(subset_df, cell) {
    strain <- subset_df$strain
    cfu <- subset_df[[cell]]
...
pair_combinations <- function(subset_df) {
    strain <- subset_df$strain
    ...
    if (length(strain) == 3) {
        ...
        # If only two objects, handle as a pair
        pair_combinations <- data.frame(strain1 = strain[1], 
                                        strain2 = strain[2], 
                                        tritagonist = NA)
        results <- pair_combinations
